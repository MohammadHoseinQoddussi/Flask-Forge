...
class Auth:
    ...
    def register(self, model=None ,  **data:dict):
        ...
        elif self.db.exists(model=model , username=self._username):
    ...
    #==============login==============
    def login(self, model:Database=None , username:str=None , password:str=None):
        if (
            not model or
            not username or
            not password
        ...
        if not check_hashing(password , user.password):
            raise VaildDataError('the username or password is not valid')
    ...
    def add_role(self, model=None, id: int = None, role: str = None):
        ...
        self.role[user.id.__str__()] = role
    ...
    def require_role(self, function: callable, model=None, role: str = None):
        ...
        def wrapper(*args, **kwargs):
            ...
            if not user or self.role.get(user.id.__str__()) != role:
                flask.abort(403)
        ...
        return wrapper
    class Signal:
        """Signal class for handling before/after callbacks."""
        
        def __init__(self):
            # Initialize with empty lambda functions to prevent AttributeError
            self.before = lambda: None
            self.after = lambda: None

        def Performance(self, before:dict=None, after:dict=None):
            """Execute before and after callbacks."""
            if not before:
                self.before()
            else:
                self.before(**before)
        ...
        def set_name_after(self, function:callable):
            """Set the callback function to execute before the main operation."""
            self.before = function

        def set_name_before(self, function:callable):
            """Set the callback function to execute after the main operation."""
            self.after = function
